/*
  ...
  It then outputs an inverted image "outImage.pgm"
*/
...
int main() {
	...
	for (int row = 0; row < h; row++) {
		for (int col = 0; col < w; col++) {
			/*
			//if ((col >= w / 2)) { //Part B right half
				//out[row][col] = 255 - img[row][col]; //Part A
			//}
			//else {
				//out[row][col] = img[row][col];
			//}

			if ((row >= h / 4 && row <= 3 * h / 4) && (col == w / 4 || col == 3 * w / 4))   //Part D
			{
				out[row][col] = 255;
			}
		...
		}										//makes out a copy of 255(white) - img
	}

	// and save this new image to file "outImage.pgm"
	writeImage(out, h*2, 2*w);
}
